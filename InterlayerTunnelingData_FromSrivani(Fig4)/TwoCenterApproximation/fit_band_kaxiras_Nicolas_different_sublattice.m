...
function =fit_band_kaxiras_Nicolas_different_sublattice(V0AA, V0AB, V3AA, V3)
...
habp  =  habp  +  tkaxirasParamsAADifferentSublattice([dif2b;cz],theta12b,theta21b) .* exp( 1j*(dot(kv,dif2b) )) ;
...
hbap  =  hbap  +  tkaxirasParamsAADifferentSublattice([dif2c;cz],theta12a,theta21a) .* exp( 1j*(dot(kv,dif2c) )) ;
...
V0_sigma_2 = 0.8216;
 ...
 habp = V0_sigma_1 * habp;
 hbap = V0_sigma_1 * hbap;
...
%  plot(kvec(3,:)*const,emod(:,ndim/2),'*')
  ...
  % plot(kvec(3,:)*const,emod(:,ndim/2),kvec(3,:)*const,emod(:,ndim/2-1))
  % plot(kvec(3,:)*const,emod(:,ndim/2+1),kvec(3,:)*const,emod(:,ndim/2+2)) 
   plot(kvec(3,:)*const,emod(:,ndim/2-1),"b-")
   plot(kvec(3,:)*const,emod(:,ndim/2),"b-")
   plot(kvec(3,:)*const,emod(:,ndim/2+1),"b-") 
   plot(kvec(3,:)*const,emod(:,ndim/2+2),"b-")

   plot(kvec(3,:)*const,emodF2G2(:,ndim/2-1),"r--")
   plot(kvec(3,:)*const,emodF2G2(:,ndim/2),"r--")
   plot(kvec(3,:)*const,emodF2G2(:,ndim/2+1),"r--") 
   plot(kvec(3,:)*const,emodF2G2(:,ndim/2+2),"r--")
