function valArray=getHkdCF_Koshino(V0_sigma, dvecT,element,realOrImag)
...
valArray = zeros(length(dvecT(:,1)),1);

maxhopping = 0;

for ddd = 1:length(valArray)
    %DFT = DFTVec(ddd);
    dvec = dvecT(ddd,:).';
    %dvectemp   =  [ 0 ; 1/sqrt(3) ] 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%t2_kaxiras = tkaxirasParamsAASameSublattice_theta(dist2,theta) ;
%t2_kaxiras = tmayouParams(dif2a,cz, V0_pi, V0_sigma, r0);
t2_kaxiras = tmayou(dif2a,cz);
...
%valueToCheck = tkaxirasParamsBApDifferentSublattice([dif2c;cz],theta12a,theta21a) .* exp( 1j*(dot(kv,dif2c) ));
